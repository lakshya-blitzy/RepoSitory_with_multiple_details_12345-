...
def capture_screenshot(driver: WebDriver, scenario_name: str, 
    ...
    try:
        ...
        logger.debug(f"Screenshot directory ensured: {screenshot_dir.resolve()}")
        ...
        logger.info(f"Screenshot captured successfully: {screenshot_path.resolve()}")
        ...
        if attach_to_allure and ALLURE_AVAILABLE:
            try:
                ...
                logger.debug(f"Screenshot attached to Allure report: {sanitized_name}")
            except Exception as allure_error:
                logger.warning(
                    f"Failed to attach screenshot to Allure report: {allure_error}",
                    exc_info=True
                )
        ...
        logger.exception(f"Failed to capture screenshot for scenario '{scenario_name}': {e}")
        return None
...
def capture_browser_logs(driver: WebDriver, scenario_name: str,
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Available log types: {available_log_types}")
        except Exception as log_types_error:
            logger.warning(f"Could not retrieve available log types: {log_types_error}")
        ...
        for log_type in log_types:
            if log_type not in available_log_types:
                logger.warning(f"Log type '{log_type}' not available, skipping")
                continue
            ...
            try:
                ...
                logger.debug(f"Captured {len(log_messages)} log entries for type '{log_type}'")
            except Exception as log_capture_error:
                logger.warning(
                    f"Failed to capture '{log_type}' logs: {log_capture_error}",
                    exc_info=True
                )
        ...
        if captured_logs:
            ...
            try:
                ...
                logger.info(f"Browser logs saved to: {log_path.resolve()}")
                ...
                if ALLURE_AVAILABLE:
                    try:
                        ...
                        logger.warning(
                            f"Failed to attach logs to Allure: {allure_error}",
                            exc_info=True
                        )
                        
            except Exception as file_error:
                logger.error(f"Failed to save browser logs to file: {file_error}", exc_info=True)
        
        return captured_logs if captured_logs else None
        
    except Exception as e:
        logger.exception(f"Failed to capture browser logs for scenario '{scenario_name}': {e}")
        return None
...
logger.debug(f"Screenshot helper module initialized (Allure available: {ALLURE_AVAILABLE})")
