...
class Config:
    ...
    def __init__(self, config_file: str = 'config/config.yaml'):
        ...
        try:
            ...
            logger.debug(f"Browser config: type={self.browser.type}, headless={self.browser.headless}")
            
            self.timeouts = TimeoutConfig(**self._raw_config['timeouts'])
            logger.debug(f"Timeout config: explicit={self.timeouts.explicit}s, page_load={self.timeouts.page_load}s")
            
            self.application = ApplicationConfig(**self._raw_config['application'])
            logger.debug(f"Application config: base_url={self.application.base_url}")
            ...
            logger.debug(f"Reporting config: formats={self.reporting.formats}")
    ...
    def _substitute_env_vars(self, yaml_content: str) -> str:
        ...
        def replace_env_var(match):
            ...
            if env_value is not None:
                logger.debug(f"Substituted environment variable: {var_name}")
                return env_value
                ...
                if var_name.endswith(('USERNAME', 'PASSWORD')):
                    logger.warning(f"Credential environment variable not set: {var_name}")
    ...
    def _validate_credentials(self) -> None:
        ...
        if not self.credentials.sales_manager_username or not self.credentials.sales_manager_password:
            logger.warning(
